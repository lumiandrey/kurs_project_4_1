...
import by.bsuir.zavadatar.andrey.teammanagerbsuir.model.db.startdata.PersonStorageInit;
...
import static by.bsuir.zavadatar.andrey.teammanagerbsuir.model.db.KorpPortalDBSchema.TypeActivityTable;
import static by.bsuir.zavadatar.andrey.teammanagerbsuir.model.db.KorpPortalDBSchema.TypeTaskTable;
...
abstract public class BaseHelper extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        db.execSQL("CREATE TABLE " + TypeActivityTable.NAME + " (" +
                TypeActivityTable.Colums.ID_TYPE_ACTIVITY + " INTEGER PRIMARY KEY AUTOINCREMENT UNIQUE, " +
    ...
    private void putDataBase(SQLiteDatabase db){
        CountryDao countryDao = new CountryDaoLite(db);
        countryDao.create(new CountryStorageInit().getData());

        CityDao cityDaoLite = new CityDaoLite(db);
        cityDaoLite.create(new CityStorageInit().getData(countryDao.reads()));

        DepartmentDao departmentDaoLite = new DepartmentDaoLite(db);
        departmentDaoLite.create(new DepartmentStorageInit().getData());

        PostDao postDaoLite = new PostDaoLite(db);
        postDaoLite.create(new PostStorageInit().getData());

        TypeActivityDao typeActivityDaoLite = new TypeActivityDaoLite(db);
        typeActivityDaoLite.create(new TypeActivityStorageInit().getData());

        TypeTaskDao typeTaskDaoLite = new TypeTaskDaoLite(db);
        typeTaskDaoLite.create(new TypeTaskSorageInit().getData());

        TypeUserDao typeUserDaoLite = new TypeUserDaoLite(db);
        ...
        PersonDao personDao = new PersonDaoLite(db);
        personDao.create(new PersonStorageInit().getData(
                userDao.reads(),
                postDaoLite.reads(),
                departmentDaoLite.reads(),
                cityDaoLite.reads()
        ));
